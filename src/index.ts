...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (uri === "memory://current-context") {
        const context = await import('./lib/context_provider.js').then(m => m.getCurrentContext(db));
        return {
            contents: [{
                uri: uri,
                mimeType: "text/plain",
    ...
    if (uri === "memory://turn-context") {
        const context = await import('./lib/context_provider.js').then(m => m.getTurnContext(db));
        return {
            contents: [{
                uri,
                mimeType: "text/plain",
